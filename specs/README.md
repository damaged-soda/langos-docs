...
职责：记录“当前版本的约束/接口/行为/技术方案”，为开发提供输入。历史决策与取舍放 `adr/`，完全废弃的内容归档到 `archive/`。

- 通用需求/设计/技术方案规格直接放 `specs/`，单仓规格放 `specs/<repo>/`（如 `specs/langos/`）。
- 命名建议：`YYYYMMDD-short-name-spec.md`（日期必填到日，避免省略）；技术方案/设计文档可使用 `YYYYMMDD-topic-solution-spec.md` 或 `YYYYMMDD-topic-design-spec.md`，命名规则一致。
