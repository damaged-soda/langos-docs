...
Archived: 2025-12-11  
Original-path: specs/langos/20251211-langos-docs-classification-spec.md  
...
- 这个文档主要解决：决策过程 / 当前约束或接口 / 远期设想 / 术语或数据模型 / 元规则 / 项目计划 / 仓库概览 里的哪一类？
- 该问题是否已有单一真相源（SOT）？如果有，优先更新原文件，而不是新建。
- 放置位置是否与 `meta/conventions.md` 的目录职责一致？若不一致，先调整规则再落盘。
