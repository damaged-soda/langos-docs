...
- 期待效果：治理/自举流程仍可参考 docs，但运行时的启动/指引文本在无 docs 内容可读的情况下也清晰可用；docs 记录蓝图与解耦原则。

## 触发意图
- 治理/自举时发现运行时说明与 docs 耦合，需梳理并调整。
...
- 更新 docs 仓（如 `overview.md`、`governance.md`、`specs/self-boot-mode-spec.md` 或新增片段）记录“蓝图→独立内核”的设计原则与维护策略。
...
- docs 仓现有治理/自举/概览文档：`../langos-docs/repos/langos/{overview.md,governance.md,specs/self-boot-mode-spec.md}` 等。
...
- 在 docs 仓写明：docs 产出蓝图，运行时与 docs 分离；治理/自举可参考 docs，但运行时不可依赖 docs。
